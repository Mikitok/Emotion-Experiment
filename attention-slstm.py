...
class Classifer(object):
    ...
    def __init__(self, config, session):
        ...
        # embedding
        self.embedding = embedding = tf.Variable(
            tf.random_normal([config.vocab_size, config.hidden_size], mean=0.0, stddev=0.1, dtype=tf.float32),
            dtype=tf.float32, name="embedding", trainable=config.embedding_trainable)
        # apply embedding
        ...
        if model_name == 'slstm':
            new_hidden_states, new_cell_state, dummynode_hidden_states = self.slstm_cell("word_slstm",
                                                                                         config.hidden_size, self.mask,
                                                                                         initial_hidden_states,
                                                                                         initial_cell_states,
                                                                                         config.layer)
            representation = new_hidden_states + tf.expand_dims(dummynode_hidden_states, axis=1)
            representation, alphas = attention(representation, config.attention_size, return_alphas=True)
            self.alpha = alphas
            ...
            softmax_w = tf.Variable(
                tf.random_normal([2 * config.hidden_size, config.num_label], mean=0.0, stddev=0.1, dtype=tf.float32),
                dtype=tf.float32, name="softmax_w")
            softmax_b = tf.Variable(tf.random_normal([config.num_label], mean=0.0, stddev=0.1, dtype=tf.float32),
            ...
            softmax_w2 = tf.Variable(
                tf.random_normal([config.hidden_size, 2 * config.hidden_size], mean=0.0, stddev=0.1, dtype=tf.float32),
                dtype=tf.float32, name="softmax_w2")
            softmax_b2 = tf.Variable(tf.random_normal([2 * config.hidden_size], mean=0.0, stddev=0.1, dtype=tf.float32),
        ...
        distribution_w = tf.Variable(
            tf.random_normal([config.hidden_size, config.num_label], mean=0.0, stddev=0.1, dtype=tf.float32),
            dtype=tf.float32, name="distribution_w")
        distribution_b = tf.Variable(tf.random_normal([config.num_label], mean=0.0, stddev=0.1, dtype=tf.float32),
                                     dtype=tf.float32, name="distribution_b")
        #
        self.logits =logits = tf.matmul(representation, distribution_w) + distribution_b
...
if __name__ == "__main__":
    ...
    for train_index, test_index in skf.split(textvec, one_labels):
        ...
        pickle.dump(((transformed_text, transformed_label)),
                    open('./run_result/' + filename + '_dataset_' + str(times), 'wb'))
        ...
        with tf.Graph().as_default(), tf.Session() as session:
            initializer = tf.random_normal_initializer(0,
                                                       0.05)  # 用正态分布产生张量的初始化器。(mean, stddev, seed=None, dtype=tf.float32)
